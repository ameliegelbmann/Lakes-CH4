...
LakeCampaignList=list(
  Bretaye=list( # include filename and desired title for each lake campaign
    list(filename="N2_Bretaye_2018_06_18", plottitle="Bretaye June 2018", hline=6.5),
    list(filename="N2_Bretaye_2018_09_03", plottitle="Bretaye Sept 2018", hline=4.7),
    list(filename="N2_Bretaye_2019_07_20", plottitle="Bretaye July 2019", hline=5.5)
  ), # from here icnlude other lakes,
  Noir=list( # include filename and desired title for each lake campaign
    list(filename="N2_Noir_2018_06_20", plottitle="Noir June 2018", hline=6.4),
    list(filename="N2_Noir_2018_09_04", plottitle="Noir Sept 2018", hline=8.8),
    list(filename="N2_Noir_2019_07_24", plottitle="Noir July 2019", hline=5.2)
  ),  
  Chavonnes=list( # include filename and desired title for each lake campaign
    list(filename="N2_Chav_2018_06_18", plottitle="Chavonnes June 2018", hline=12.5),
    list(filename="N2_Chav_2018_09_05", plottitle="Chavonnes Sept 2018", hline=7.5),
    list(filename="N2_Chav_2019_07_23", plottitle="Chavonnes July 2019", hline=11.8)
  ),
  Lioson=list( # include filename and desired title for each lake campaign
    list(filename="N2_Lioson_2018_06_24", plottitle="Lioson June 2018", hline=22.9),
    list(filename="N2_Lioson_2018_08_30", plottitle="Lioson Sept 2018", hline=21.8),
    list(filename="N2_Lioson_2019_07_17", plottitle="Lioson July 2019", hline=16.4)
  )
)
...
#3.
plotCampaign <- function(LakeCampaign, PlotTitle, HLine) {
  ...
  y2 <- LakeCampaign[["Sbeox0PS"]] # O2
  y3 <- LakeCampaign[["Tv290C"]] # T°
  y4 <- LakeCampaign[["CH4"]] # CH4 measured by Chou Team, use x2
  y5 <- LakeCampaign[["Par_f"]] #PAR = Kd
  
  # Find where Par_f becomes <= 1%
  # for (yindex in seq_along(y5)) { 
  #   # yindex is a temporary variable for each of the indexes in y2 
  #   # y2 has length 498, as such, yindex will take value 1 for the first loop, 
  #   # increasing +1 value foreach loop until 498. 
  #   # This is done to evaluate at which position does y2 change from >1% to <1% (yindex+1)
  #   # then, this index is used to retrieve the corresponding x value in x1 (x1[yindex+1])
  #   if (y5[yindex] > 1 & y5[yindex+1] <= 1) {
  #     # print(yindex+1)
  #     # print(paste(y2[yindex+1],": ", x1[yindex+1])) 
  #     hline=x1[yindex+1]
  #     break
  #   }
  # }  
  ...
  # axis titles
  y1text=expression(paste("Algae"," (µg L"^-1,")"))
  y1legend=expression(paste("Algae"))
  
  y2text=expression(paste("O"[2]," (%)"))
  y2legend=expression(paste("O"[2]))
  ...
  LoopList <- list(
    list(xval=x3, yval=y1, ytext=y1text, ylegend=y1legend, linecolor=y1color, linetypelty=1, axisside=1, axispostion=1, makepoints=F),  # N2
    list(xval=x1, yval=y2, ytext=y2text, ylegend=y2legend, linecolor=y2color, linetypelty=1, axisside=1, axispostion=5, makepoints=F),  # Kd
    list(xval=x1, yval=y3, ytext=y3text, ylegend=y3legend, linecolor=y3color, linetypelty=1, axisside=3, axispostion=1, makepoints=F),   # T°
    list(xval=x2, yval=y4, ytext=y4text, ylegend=y4legend, linecolor=y4color, linetypelty=2, axisside=3, axispostion=5, makepoints=T)  # CH4
  ...
  # Draw Hline (when Kd becomes 1%)
  abline(h=hline, col="gray", lwd=plotlwd)
...
# 6. make all plots and add legends ----
for (Lake in LakeCampaignList) {  # prend chaque lake dans la "boite" LakeCmpaignList
  ...
  for (Campaign in Lake) {  #prend chaque campagne de lake pour faire des trucs apres:
    ...
    hline=Campaign[["hline"]]   #Give title
    
    lakecampaign=readCampaign(filename)
    testylisty=plotCampaign(lakecampaign, plottitle, hline)
  }
